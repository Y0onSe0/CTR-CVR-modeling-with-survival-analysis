...
unlink(file.path(Sys.getenv("R_LIBS_USER"), "00LOCK*"), recursive = TRUE, force = TRUE)

install.packages("xfun", type="binary")
install.packages("knitr", type="binary")
install.packages("rmarkdown", type="binary")
library(rmarkdown)
rmarkdown::render(system.file("rmd/hw01.Rmd", package="rmarkdown"), output_format="html_document")
...
### 1) has_prev_click, tslc 만들기 (train과 동일)
...
#step.model <- step(null.model, scope = list(upper = formula(mod_ctr1)), direction = "both")

summary(step.model) 

# 상호작용 term 추가하기 
step2<-update(step.model, . ~ .  + cost_log*tslc_log)
summary(step2)


# 상호작용 term 추가하기
step3<-update(step2, . ~ .  + cost_log*has_prev_click)
summary(step3)
